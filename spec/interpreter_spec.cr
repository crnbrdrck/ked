...
describe Ked::Interpreter do
  describe "interpret" do
    it "should be successfully be able to handle an expression of NUMBER PLUS NUMBER and return the correct value" do
      interpreter = Ked::Interpreter.new "remember €x = 3 plus 3 like"
      interpreter.interpret
      interpreter.global_scope["x"].should eq 6
    end

    it "should be successfully be able to handle an expression of NUMBER AWAY_FROM NUMBER and return the correct value" do
      interpreter = Ked::Interpreter.new "remember €x = 3 awayFrom 7 like"
      interpreter.interpret
      interpreter.global_scope["x"].should eq 4
    end

    it "should be successfully be able to handle an expression of NUMBER TIMES NUMBER and return the correct value" do
      interpreter = Ked::Interpreter.new "remember €x = 7 times 3 like"
      interpreter.interpret
      interpreter.global_scope["x"].should eq 21
    end

    it "should be successfully be able to handle an expression of NUMBER INTO NUMBER and return the correct value" do
